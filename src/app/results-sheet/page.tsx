...
export const metadata: Metadata = {
  title: "********** Result Sheet **********",
  description:
    "Play gold win, see your number result here with play gold win. play gold win",
  keywords: [
    "PlayGoldWin",
    "Play Gold Win Result Sheet",
    "result sheet",
    "playgoldwin resultsheet",
    "result sheet play gold win",
    "Play Gold Win",
    "resultsheet playgoldwin",
    "resultsheetplaygoldwin",
    "PlayGolddWin",
    "PlayGoldWin",
    "PlayGoldWin",
  ],
  ...
  openGraph: {
    ...
    description:
      "Playgoldwin is your trusted source for the latest Playgoldwin lottery results.",
  },
  twitter: {
...
export default async function Home({
  ...
  if (moment(date, "YYYY-MM-DD", true).isValid()) {
    const selectedIST = moment.tz(date, "YYYY-MM-DD", tz);

    if (selectedIST.format("YYYY-MM-DD") === todayIST) {
      // Today in IST → use current IST time
      const nowIST = moment().tz(tz);
      finalDate = moment.tz(
        `${date} ${nowIST.format("HH:mm:ss")}`,
        "YYYY-MM-DD HH:mm:ss",
        tz
      )
        .utc()
    ...
    <div className="min-h-screen bg-[#00244a] text-white flex flex-col items-center">
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="flex gap-1 mt-10 translate-y-5 items-center justify-center scale-50 sm:scale-60 md:scale-75 lg:scale-90 xl:scale-100 origin-top-left w-[200%] sm:w-[167%] md:w-[133%] lg:w-[111%] xl:w-full">
          <span className="text-lg font-semibold ms-1 text-[#f2e70f]">
            Select Date:
          </span>
        ...
        <div className="w-full flex justify-between px-2">
          <div>
            <Link
              href={"/"}
              className="text inline-block text-xs font-bold text-[#f2e70f]"
            >
              Home
            </Link>
          </div>
                ...
                <table className="table-fixed pb-3 w-full font-bold border border-gray-400 text-[clamp(9px,1.8vw,14px)]">
                  <thead>
                    <tr className="bg-[#01244a] text-center">
                      <th className="border border-gray-400 w-14">Draw Time</th>
                      ...
                      <tr key={item.id} className="border text-center">
                        ...
                        <td
                          className="
                            ...
                            text-[2.9vw]
                            [@media(min-width:345px)]:text-[3.1vw]
                            [@media(min-width:385px)]:text-[3.2vw]
                            [@media(min-width:435px)]:text-[3.2vw]
                            [@media(min-width:450px)]:text-[3.2vw]
                            [@media(min-width:480px)]:text-[3.2vw]
                            sm:text-[3.2vw]
                            md:text-[14px]
                          "
                        >

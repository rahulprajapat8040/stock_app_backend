...
export default async function Home({
  ...
  const slots: IStockInterface[] = await apiCall(`/stock/get-stock-till-now?date=${finalDate}`);
  return (
    <div className="min-h-screen bg-[#00244a] text-white flex flex-col items-center">
      <div className="w-full max-w-7xl mx-auto">
        ...
        {
          <div className="flex text-xs ms-5 mt-2 justify-center gap-1">
            <span>Result of Date :</span>
            <span className="text-[#f2e70f] text-[10px] mt-px">{date ? moment(date).format("DD.MM.YYYY") : moment().format("DD.MM.YYYY")}</span>
          </div>
        }
                  ...
                  <tbody className="bg-[#01244a]">
                    {
                      slots.map((item) => {
                        return (
                          <tr key={item.id} >
                            <td className="border-2 border-gray-400 px-2 whitespace-nowrap text-center" style={{ borderWidth: '3px', borderColor: '#9ca3af', borderStyle: 'solid' }}>{moment(item.stockTime).format("hh:mm A")}</td>
